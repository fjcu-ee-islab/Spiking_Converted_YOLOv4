...
router = "%s/yolov4_fju_448_2000_last.weights"%(os.getcwd())

#read .cfg .weights
net = cv2.dnn.readNetFromDarknet("/home/shon/code/fju_event_pedestrian_detection/CSRDCF_final/yolov4_fju_448_2000.cfg",router)
...
#讀取分類檔、設定標記框顏色
classes = [line.strip() for line in open("/home/shon/code/fju_event_pedestrian_detection/human_label_train/fju.names")]
colors = [(0,0,255),(255,0,0),(0,255,0)]

#先讀取要測試圖片
images_list = []
for img in os.listdir("/home/shon/code/fju_event_pedestrian_detection/label_final/test"):
    if img.endswith(".jpg"):
...
for st in images_list:
    ...
    boxes = []
    boxes2 = []
    img_st = "/home/shon/code/fju_event_pedestrian_detection/label_final/test/%s.jpg"%(st)
    txt_st = "/home/shon/code/fju_event_pedestrian_detection/label_final/test/%s.txt"%(st)
    print(img_st)
    print(txt_st)
    ...
    #存取檔案
    result_st = "%s/result_last/%s_og.jpg"%(os.getcwd(),st)
    result_st2 = "%s/result_last/%s_result.jpg"%(os.getcwd(),st)
...
router = "%s/result_last/result.txt"%(os.getcwd())
f = open(router,"w")
