...
int main()
{
	...
	Mat frame, img, crop_img, blur, gussian ,wtb ,wtb_blur;
	Rect roi;
	...
	VideoWriter writer3("C:\\Users\\shon\\Desktop\\JAER\\9\\scale8\\whitetoblack.avi", CV_FOURCC('X', 'V', 'I', 'D'), 30.0, Size(438, 438));
	VideoWriter writer4("C:\\Users\\shon\\Desktop\\JAER\\9\\scale8\\whitetoblack_blur.avi", CV_FOURCC('X', 'V', 'I', 'D'), 30.0, Size(438, 438));
	//先將圖像儲存到frame
	...
	namedWindow("white to black", CV_WINDOW_AUTOSIZE);
	//代表有讀取到影像
	while (capture.read(frame))
	{
		...
		//中值濾波
		medianBlur(crop_img, wtb_blur, 5);
		...
		imshow("white to black", wtb_blur);
		//等待33ms類似於30FPS
		waitKey(5);
		//儲存影片
		writer << crop_img;
